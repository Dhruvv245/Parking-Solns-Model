...
def checkParkingSpace(imgPro):
    ...
    if parking_status != prev_parking_status:
        ...
        # Send parking space data to backend
        # parking_data = [{"position": posList[i], "occupied": status} for i, status in enumerate(parking_status)]
        try:
        # Send data to the Node.js backend
            response = requests.get('http://127.0.0.1:3000/api/v1/parkings/5c88fa8cf4afda39709c2951')
